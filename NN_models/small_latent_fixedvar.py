...
class ConditionalAutoEncoder(pl.LightningModule):
    ...
    def __init__(self, schedule, learning_rate):
        ...
        self.data_process = nn.Sequential(
            nn.Linear(3840, 3840),  # Fully connected
            nn.ReLU(),
            nn.Linear(3840, 1024),
            nn.ReLU(),
            nn.Linear(1024, 512),
    ...
    def forward(self, batch):
        ...
        r1_means = self.r1_mu(data_flat)
        r1_stds = torch.exp(self.r1_sigma(data_flat))
...
if __name__ == '__main__':
    ...
    L = frange_cycle_linear(int(np.ceil(0.1 * training_size / args.batch_size)), 0, 1, 1, 0.5)
